...
export const metricsRouter = createTRPCRouter({
	getMetrics: publicProcedure.query(async ({ ctx }) => {
		...
		// ⚡ BOLT OPTIMIZATION:
		// Execute all independent Prisma queries concurrently with Promise.all
		// This prevents N+1 sequential bottlenecks and speeds up endpoint execution time.
		const [
			applied,
			confirmed,
			checkedIn,
			...
			ctx.prisma.presence.groupBy({
				by: ["label"],
				_sum: { value: true },
				having: { label: { _count: { gte: threshold * 10 } } },
			}),
			ctx.prisma.hacker.groupBy({
				by: ["gender"],
				_count: { gender: true },
				having: { gender: { _count: { gte: threshold } } },
			}),
			ctx.prisma.hacker.groupBy({
				by: ["raceEthnicity"],
				_count: { raceEthnicity: true },
				having: { raceEthnicity: { _count: { gte: threshold } } },
			}),
			ctx.prisma.hacker.groupBy({
				by: ["age"],
				_count: { age: true },
			...
			ctx.prisma.hacker.groupBy({
				...
				having: { travelOrigin: { _count: { gte: threshold } } },
			}),
			ctx.prisma.hacker.groupBy({
				by: ["educationLevel"],
				_count: { educationLevel: true },
				having: { educationLevel: { _count: { gte: threshold } } },
			}),
			ctx.prisma.hacker.groupBy({
				by: ["referralSource"],
				_count: { referralSource: true },
				having: { referralSource: { _count: { gte: threshold } } },
			}),
			ctx.prisma.hacker.groupBy({
				by: ["major"],
				_count: { major: true },
				having: { major: { _count: { gte: threshold } } },
			}),
		]);

		const presences = presencesRaw.reduce((sum, presence) => sum + presence.value, 0);

		// Reuse confirmed and walkIn instead of making 2 extra count queries
		const attendees =
			confirmed +
			walkIn +
			ORGANIZER_COUNT +
