...
const Hackers: NextPage = () => {
	...
	// 		: hackers?.filter(
	// 				hacker => {
	// 					if ("education" in hacker && hacker.education) {
	// 						if ("university" in hacker.education) {
	// 							hacker.education?.university?.toLowerCase().includes(search.toLowerCase())
	// 						}
	// 						if ("studyProgram" in hacker.education) {
	// 							hacker.education?.studyProgram?.toLowerCase().includes(search.toLowerCase())
	// 						}
	// 					}
	// 					if ("personalInfo" in hacker && hacker.education) {
	// 						if ("university" in hacker.education) {
	// 							hacker.education?.university?.toLowerCase().includes(search.toLowerCase())
	// 						}
	// 						if ("studyProgram" in hacker.education) {
	...
	// 						}
	// 					}
	// 					"education" in hacker ? hacker.education?.university?.toLowerCase().includes(search.toLowerCase()) : null ||
	// 					"education" in hacker ? hacker.education?.studyProgram?.toLowerCase().includes(search.toLowerCase()): null ||
	// 					"personalInfo" in hacker ? `${hacker.personalInfo.firstName} ${hacker.personalInfo.lastName}`.toLowerCase().includes(search.toLowerCase()) : null
	// 				}
	// 			);

	const filteredQuery = search.length === 0
	? hackers
	: hackers?.filter(hacker => {
		const education = "education" in hacker && hacker.education !== null ? hacker.education : null;
		...
		return university.toLowerCase().includes(searchLower) ||
			   studyProgram.toLowerCase().includes(searchLower) ||
					...
					>
						{filteredQuery?.map(hacker => (
							<Card
								key={hacker.id}
								id={hacker.id}
								firstName={"personalInfo" in hacker && hacker.personalInfo !== null ? hacker.personalInfo?.firstName : '' }
								lastName={"personalInfo" in hacker && hacker.personalInfo !== null ? hacker.personalInfo?.lastName : '' }
								university={"education" in hacker && hacker.education !== null ? hacker.education?.university ?? '' : '' }
								studyProgram={"education" in hacker && hacker.education !== null ? hacker.education?.studyProgram ?? '' : '' }
							/>
						))}
					</div>
				</div>
