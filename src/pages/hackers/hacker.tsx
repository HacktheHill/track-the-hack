...
const HackerView = ({ hackerData, presenceData: { id: _, hackerInfoId, ...presenceData } }: HackerViewProps) => {
	...
	const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
		...
		const parse = walkInSchema.extend({
			id: z.string()
		}).safeParse(data)
				...
				{Object.keys(groupedData).map((category, index) => (
					<div key={index}>
						<div className="flex justify-center">
							<h2 className="self-center font-[Coolvetica] text-2xl font-normal text-dark px-3 py-2">
								{category}
							</h2>
						</div>
							...
							<div key={itemIndex} className={paragraphClass}>
								...
								{item.type === "select" ? (
									<select
										id={item.name}
										name={item.name}
										className="w-[50%] rounded-[100px] border-none bg-background1 px-5 py-2 font-rubik text-dark shadow-md transition-all duration-500 hover:bg-background1/50 col-md-6"
									...
									<input
										...
										name={item.name}
										type={item.type}
										className="rounded-[100px] border-none bg-background1 px-4 py-2 font-rubik text-dark shadow-md transition-all duration-500 hover:bg-background2	/50 w-[50%] outline-none"
										defaultValue={item.default_value !== null ? item.default_value : (item.type === 'select' ? '' : `No ${item.label}`)}
										value={inputValues[item.name] || ''}
										onChange={(e) => {
											handleInputChange(item.name, e.target.value);
										}}
