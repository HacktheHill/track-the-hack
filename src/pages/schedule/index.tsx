...
const Schedule: NextPage = () => {
	...
	// Bolt optimization: Memoize expensive array operations that run on every render
	const events = useMemo(() => {
		...
		return query.data
			...
			.reduce((acc, event, i, array) => {
				if (
					array[i]?.start.toLocaleDateString(dateLocale) === array[i - 1]?.start.toLocaleDateString(dateLocale)
				) {
	...
	}, [query.data, tab, currentTime, dateLocale]);
