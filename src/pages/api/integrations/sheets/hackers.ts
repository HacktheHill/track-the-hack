...
import { provisionHackers } from "../../../../server/services/hacker-lifecycle";

const repository = new PrismaHackerLifecycleRepository(prisma);

export default async function provision(req: NextApiRequest, res: NextApiResponse) {
	...
	try {
		const result = await provisionHackers(repository, req.body);
		...
		console.error("Sheet provisioning failed");
